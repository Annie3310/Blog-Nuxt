...
export default {
  ...
  created() {
    ...
    let promise = api.getBlog(id);
    console.log(id)
    let _this = this
    promise.then(res => {
      _this.isLoading = false;
      if (res.data.code === 'B0001') {
        _this.isArticleEmpty = true;
        _this.commentVisible = false;
        return;
      }
      let result = res.data.result;
      document.title = result.title
      _this.article.body = result.body;
      _this.article.title = result.title;
      _this.article.toc = result.toc;
      let time = new Date(result.createdAt);
      _this.article.createdAt = time.getFullYear() + ' 年 ' + (time.getMonth() + 1) + ' 月 ' + time.getDate() + ' 日';
    })
  },
  mounted() {
